...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    ...
    addTaskButton: function (win) {
        return this.tbPanel.addButton(win, 'taskbuttons');
    },
...
Ext.ux.TaskButtonsPanel = Ext.extend(Ext.BoxComponent, {
    ...
    initComponent: function () {
        ...
        this.stripWrap = Ext.get(this.el).createChild({
            cls: 'taskbuttons-strip-wrap',
            cn: {
                tag: 'ul',
                cls: 'taskbuttons-strip'
            }
        });
        this.stripSpacer = Ext.get(this.el).createChild({
            cls: 'taskbuttons-strip-spacer'
        });
        this.strip = new Ext.Element(this.stripWrap.dom.firstChild);
        this.edge = this.strip.createChild({
            tag: 'li',
            cls: 'taskbuttons-edge'
        });
    ...
    createScrollers: function () {
        ...
        var sl = this.el.insertFirst({
            cls: 'taskbuttons-scroller-left'
        });
        sl.setHeight(h);
        sl.addClassOnOver('taskbuttons-scroller-left-over');
        ...
        var sr = this.el.insertFirst({
            cls: 'taskbuttons-scroller-right'
        });
        sr.setHeight(h);
        sr.addClassOnOver('taskbuttons-scroller-right-over');
    ...
    scrollToButton: function (item, animate) {
        if (typeof(item.el.dom) != 'undefined') {
    ...
    updateScrollButtons: function () {
        var pos = this.getScrollPos();
        this.scrollLeft[pos == 0 ? 'addClass' : 'removeClass']('taskbuttons-scroller-left-disabled');
        this.scrollRight[pos >= (this.getScrollWidth() - this.getScrollArea()) ? 'addClass' : 'removeClass']('taskbuttons-scroller-right-disabled');
    }
});
/**
...
Ext.ux.TaskBar.TaskButton = function (win, el) {
    ...
    Ext.ux.TaskBar.TaskButton.superclass.constructor.call(this, {
        ...
        template: new Ext.Template('<table cellspacing="0" class="x-btn {3}"><tbody><tr>', '<td class="taskbutton-left"><i>&#160;</i></td>', '<td class="taskbutton-center"><em class="{5} unselectable="on">', '<button class="x-btn-text {2}" type="{1}" style="height:28px;">{0}</button>', '</em></td>', '<td class="taskbutton-right"><i>&#160;</i></td>', "</tr></tbody></table>")
    });
};
Ext.extend(Ext.ux.TaskBar.TaskButton, Ext.Button, {
    ...
    closeWin: function (context, e, win) {
        if (!win.isVisible()) {
            win.show();
        } else {
            win.restore();
